/*
 * 参考文章： https://juejin.cn/post/6968630178163458084#heading-27
 */
...
function myAxios(axiosConfig: any, customOptions: ICustomOptions) {
 ...
 * @param {*} error
 */
function httpErrorStatusHandle(error: IAxiosError) {
  // 处理被取消的请求
 ...
 * @param {*} _options
 */
function closeLoading(_options: ICustomOptions) {
 ...
 * @param {*} config
 */
function addPending<T>(config: AxiosRequestConfig<T>) {
 ...
 * @param {*} config
 */
function removePending<T>(config: AxiosRequestConfig<T>) {
  const pendingKey = getPendingKey(config);
  if (pendingMap.has(pendingKey)) {
 ...
 * @returns
 */
function getPendingKey<T>(config: AxiosRequestConfig<T>) {
